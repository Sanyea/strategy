...
- 微服务拆分、Redis 会话缓存（refresh 会话落库即可）
...
- 门面 service 是跨表事务收口点（`doInTransaction` 钩子，见 common 层事务架构）
...
│   └── dto/    RegisterDTO / LoginDTO / RefreshDTO / TokenVO
├── profile/                   # 能力包：个人资料
...
├── common/
│   ├── auth/                  JwtUtil / UserContext / PasswordEncoder(BCrypt 封装) / TotpUtil
...
|-------|------|-------------|-----|------|
| accessToken | JWT（HS256） | `userId, userType, jti, deviceId, exp` | 30 min | 客户端内存/Header |
| refreshToken | 不透明随机串（`SecureRandom` 32B） | 无 | 14 天 | `ums_user_login_device` 会话行 |

- `jti` = 会话行 id。吊销会话 = 失效对应会话行
- accessToken 无状态校验（JWT 验签），请求不打库；refreshToken 服务端校验会话行是否有效（`is_current=1`、`expire_time` 未过、非逻辑删除）
...
preHandle:
  白名单路径 → 放行
      /auth/login /auth/register /auth/refresh /actuator/** /error
  解析 Authorization: Bearer <accessToken>
    ├─ 缺失/格式错/验签失败/过期 → 抛 401（BizException + ResultCode.UNAUTHORIZED）
    ├─ 成功 → 取 claims(userId/userType/jti/deviceId)
    └─ 查用户状态：userStatus ∈ {冻结, 注销} → 抛 403/410
...
- 401 分支补入 `GlobalExceptionHandler`（现仅 4xx/5xx 语义，缺 401 码）
- `ResultCode` 新增：`UNAUTHORIZED(401)`、`TOKEN_EXPIRED(401)`、`ACCOUNT_LOCKED(403)`、`ACCOUNT_DISABLED(403)`、`ACCOUNT_DELETED(410)`、`DEVICE_KICKED(401)`、`MFA_REQUIRED(403)`
...
- MFA：TOTP（RFC 6238），JDK 内置 `HmacSHA1` + Base32，不引第三方库（`TotpUtil`）。`mfa_secret` 存 Base32 密钥，校验 30s 时间窗口 ±1
...
POST /auth/login {account, password, deviceInfo}
  ...
  3. 密码错误 → security.passwordErrorCount +1；达阈值(如 5) 置 lockTime（锁 30min）
  4. userStatus 校验：冻结/注销 → 拒绝
  5. DeviceService.createSession → 会话行（置旧 is_current 会话保留，新会话为当前）
  6. 更新 user.lastLoginTime / lastLoginIp / lastLoginDeviceId
  7. 签发双 Token
```
...
POST /auth/refresh {refreshToken, deviceId}
  ↓ AuthService.refresh
  1. 查会话行（jti）：不存在/已失效/deviceId 不符 → 401（TOKEN_EXPIRED）
  2. 轮换：作废旧 refreshToken，写新会话行（复用或更新）
  3. 签发新 accessToken
...
```
POST /auth/logout → DeviceService.invalidateSession(jti)  （置 is_current=0 / 标记失效）
POST /users/me/password → SecurityService.changePassword
  1. 校验旧密码
  2. 新密码策略 + PasswordEncoder.encode
  3. 更新 user.password
  4. 吊销该账号全部会话行（踢所有设备，含当前）
...
PUT  /admin/users/{id}/status      → AdminUserService.changeStatus：冻结/解冻/注销
POST /admin/users/{id}/reset-password → AdminUserService.resetPassword：生成随机密码 + PasswordEncoder + 吊销全部会话
...
**决策：允许复用。** 逻辑删除行仍占 DB 唯一键，删除时对唯一标识追加后缀释放。
...
- 注销走 `ums_user.deleted=1`（逻辑删除）+ 唯一标识后缀化，二者同事务
- 重新注册同名/同手机/同邮箱：DB 唯一键已释放，`@TableLogic` 已过滤旧行，可直接复用
...
```sql
-- 登出/吊销按 jti(主键) 查，已覆盖；补 user_id + is_current 联合索引便于设备列表
ALTER TABLE ums_user_login_device ADD KEY idx_user_current (user_id, is_current);
```
...
| 10 逻辑删除+唯一键占用 | 注销后缀释放 + 列宽放宽 | 批5（注销功能）/批6（DDL） |
| 11 空手机/邮箱撞唯一键 | 空值 NULL 化 + 归一化 | 批6（DDL） |
| 12 `ext_info` JSON | 自定义 TypeHandler（Jackson 3）或应用层 JSON 工具 | 批6 |
| 会话过期残留 | refresh 过期自动失效；定期清理 `login_device`（`expire_time < now` 且 `is_current=0`） | 批4 |
...
`AbstractBaseService` 模板方法 + `beforeXxx/afterXxx` 钩子已就绪，门面在 `doInTransaction` 内编排跨表写；单表审计填充经 `MetaObjectHandler`（`MybatisPlusConfig`）插入/更新钩子完成。本设计不新增模板方法模式实例，沿用现状。

## 11. 分阶段实施计划

| 批次 | 内容 | 涉及 |
|------|------|------|
| 批1 认证主链 | 依赖（jjwt/jbcrypt）+ `JwtUtil`/`UserContext`/`PasswordEncoder` + `TokenAuthInterceptor` + `WebMvcConfig` + `AuthController`/`AuthService`（注册/登录/刷新/登出）+ `ResultCode` 401 系 + `GlobalExceptionHandler` 401 分支 + `MetaObjectHandler` 审计填充 | auth 包 + common |
| 批2 个人中心 | `ProfileController`/`ProfileService`（查/改 me + profile）+ 脱敏 `UserInfoVO` + DTO 校验 | profile 包 |
| 批3 账号安全 | 改密/重置/绑定解绑手机邮箱/MFA（`SecurityController`/`SecurityService` + `TotpUtil`） | security 包 |
...
| 批6 收尾 | DDL（列宽放宽 + 空值 NULL 化 + 索引）+ `ext_info` TypeHandler + 会话清理任务 + 字段级审计（若定方案） | sql + common + 各包 |
...
- **会话清理**：`login_device` 增长需定期清理（Spring `@Scheduled` 或外部任务），批4 挂
